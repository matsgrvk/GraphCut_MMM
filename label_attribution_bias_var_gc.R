...
for(m in 1:length(model_names)){
  ...
  gc_result <- graphcut(var.present = var_present,
...
for(m in 1:length(model_names)){
  ...
  bias_var_result <- bias_var(var.future = var_future,
                              gc.data = gc_result)
}